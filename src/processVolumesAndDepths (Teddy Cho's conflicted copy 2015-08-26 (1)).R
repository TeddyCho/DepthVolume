...
enrichDepthVolumeData <- function(aData, myDepthStyle){
  ...
  aData$AverageDepth_Dollars <- aData[, paste("AverageDepth_Dollars_", myDepthStyle, sep="")]
  aData$AverageDepth_Shares <- aData[, paste("AverageDepth_Shares_", myDepthStyle, sep="")]
...
createCoeffPlot <- function(theSeries, myOutputFolder, myDepthStyle){
  mySymbols = unique(theSeries$Symbol)
  myDates <- unique(theSeries$Date)
  ...
  for(sym in mySymbols){
    ...
    ggplot(data=myCoeffDF, aes(x=date, y=beta)) +
  ...
  ggplot(myCoeffDF, aes(x=Beta, fill=depthMethod)) + 
    ...
    geom_vline(aes(yintercept=1), colour = "green") +
    facet_wrap(~variable, ncol=1, scales="free") + 
}
...
createCoeffPlot(theSeries, myOutputFolder, myDepthStyle)
...
myFilteredSeries <- theSeries
    lteredSeries[is.na(myFilteredSeries)] <-0
...
CoefficientDF <- getCoefficients(theSeries, FALSE)
myCoefficientDF[mapply(is.infinite, myCoefficientDF)] <- NA
